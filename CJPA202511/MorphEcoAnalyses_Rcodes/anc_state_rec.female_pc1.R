...
phenotype_data <- read.csv("G:/longxi/longxibioinfo/PhenotypeStatistics/PCA_anc/PCA_Female_MeanPC1_by_species.csv")
...
if (any(is.na(svl_vector))) {
  print("Warning: Missing PC1 data for species:")
  print(phenotype_data[is.na(svl_vector), ])
  stop("Check data file and complete missing PC1 values.")
}
...
if (!inherits(ou_model, "try-error")) {
  ...
  cat("\nOU model fitting failed, comparing only BM and White models\n")
